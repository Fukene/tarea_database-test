...
const EditItem = (props) => {
    ...
    const actualizarFactura = async (invoiceId) => {
        try {
            ...
            router.push(`/facturas/facturas/${invoiceId}`)
    ...
    useEffect(() => {
        ...
        setFechaEntrega(factura.fechaEntrega);
        setFechaRecepcion(factura.fechaRecepcion);

        setClienteNombre(factura.cliente.propietario.nombre);
        setCedula(factura.cliente.propietario.cedula)
        setLugarOperaciones(factura.cliente.propietario.lugarOperaciones)
        setClienteDireccion(factura.cliente.propietario.direccion)
        setClienteTelefono(factura.cliente.propietario.telefono)
        setClienteEmail(factura.cliente.propietario.email)

        setExplotador(factura.cliente.explotador)

        setRepresentanteLegalNombre(factura.cliente.representanteLegal.nombre)
        setRepresentanteLegalDireccion(factura.cliente.representanteLegal.direccion)
        setRepresentanteLegalTelefono(factura.cliente.representanteLegal.telefono)
        setRepresentanteLegalEmail(factura.cliente.representanteLegal.email)

        setMatriculaAeronave(factura.aeronave.datosAeronave.matricula)
        setMarcaAeronave(factura.aeronave.datosAeronave.marca)
        setModeloAeronave(factura.aeronave.datosAeronave.modelo)
        setNumeroSerieAeronave(factura.aeronave.datosAeronave.numeroDeSerie)
        setAnoFabricacion(factura.aeronave.datosAeronave.anoFabricacion)
        setCategoriaAeronave(factura.aeronave.datosAeronave.categoria)
        setPesoBruto(factura.aeronave.datosAeronave.pesoBruto)
        setPesoVacio(factura.aeronave.datosAeronave.pesoVacio)
        setFechaUltimoPeso(factura.aeronave.datosAeronave.fechaUltimoPeso)
        setHorasAeronave(factura.aeronave.datosAeronave.horasTotales)
        setTipoInspeccion(factura.aeronave.programaDeMantenimiento.tipoInspeccion)

        setFechaMantenimiento(factura.aeronave.programaDeMantenimiento.fecha)
        setHorasMantenimiento(factura.aeronave.programaDeMantenimiento.horas)
        setCategoriaMantenimiento(factura.aeronave.programaDeMantenimiento.categoria)
        setUtilizacion(factura.aeronave.programaDeMantenimiento.utilizacion)

        setTiempoReparacion(factura.tiempoReparacion)
    ...
    return <div className="main_container">
        <div className="new_invoice">
            <div className="new_invoice-header">
                <FontAwesomeIcon onClick={() => router.push(`/facturas/facturas/${factura.id}`)} className="back_btn" icon={icon({ name: 'arrow-left', style: 'solid' })} />
            ...
            <div className="new_invoice-body">
                ...
                <div className="new_invoice_btns" style={{ justifyContent: 'end' }}>
                    <button className="discard_btn" onClick={() => router.push(`/facturas/facturas/${factura.id}`)}>Cancelar</button>
                    <button className="send_btn" onClick={() => actualizarFactura(factura.id)} >Guardar cambios</button>
                </div>
            </div>
        </div>
    </div>
...
export async function getStaticPaths() {
    ...
    const db = client.db();
    const coleccionFacturas = db.collection('facturas');

    const facturas = await coleccionFacturas.find({}, { _id: 1 }).toArray();

    return {
        fallback: 'blocking',
        paths: facturas.map((invoice) => ({
            params: {
...
export async function getStaticProps(context) {
    ...
    const factura = await facturas.findOne({ _id: ObjectId(invoiceId) })

    const informacionCliente = await clientes.findOne({ _id: factura.clienteId });
    const informacionAeronave = await aeronaves.findOne({ _id: factura.aeronaveId })
    ...
    return {
        props: {
            data: {
                ...
                items: factura.items,
                total: factura.total,
                tiempoReparacion: factura.tiempoReparacion,
