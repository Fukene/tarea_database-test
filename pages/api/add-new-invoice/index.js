...
async function handler(req, res) {
    ...
    if (req.method === 'POST') {
        ...
        await facturas.insertOne(factura)

        const ultimaFactura = await facturas.find({}).sort({ _id: -1 }).limit(1).toArray();

        await clientes.updateOne(criterioBusquedaCliente,
            {
                $push: {
                    "facturas": ultimaFactura[0]._id
                }
            })
        ...
        const criterioBusquedaFactura = { "_id": ultimaFactura[0]._id }

        await facturas.updateOne(criterioBusquedaFactura,
            {
                $set: {
